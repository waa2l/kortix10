...
export default function DisplayScreen() {
  ...
  // 1. الاستماع للطوارئ والتحويلات
  useEffect(() => {
    if (!isAuthenticated) return;

    // A. قناة الطوارئ من جدول queue
    ...
    // B. قناة التحويلات (Broadcast)
    const alertChannel = supabase.channel('control-alerts')
      .on('broadcast', { event: 'clinic-transfer' }, (payload) => {
        // التحقق مما إذا كانت العيادة المحول إليها موجودة في هذه الشاشة (اختياري)
        // هنا نعرض التنبيه للجميع
  ...
  // 2. الاستماع للنداءات العادية (تحديثات الجدول)
  useEffect(() => {
  ...
  const triggerTransferAlert = async (data: any) => {
    setNotification({
      show: true,
      message: `( تحويل ) العميل رقم ${toArabicNumbers(data.ticketNumber)}`,
      subtext: `يرجى التوجه إلى ${data.toClinicName}`,
      type: 'transfer'
    });
    ...
    if (!isMuted) {
      const audioFiles = [
        ...
        `/audio/clinic${data.toClinicNumber}.mp3` // نطق العيادة الجديدة
      ];
    ...
    <div className={`min-h-screen bg-gray-900 text-white overflow-hidden relative flex flex-col font-cairo ${notification?.type === 'emergency' ? 'animate-pulse' : ''}`}>
      ...
      <div className={`fixed top-0 left-0 w-full z-50 transform transition-transform duration-500 ease-out ${notification ? 'translate-y-0' : '-translate-y-full'}`}>
        <div className={`shadow-2xl border-b-4 border-white p-6 flex justify-between items-center ${
          notification?.type === 'emergency' ? 'bg-gradient-to-r from-red-600 to-red-800' : 
          notification?.type === 'transfer' ? 'bg-gradient-to-r from-indigo-600 to-blue-800' :
          'bg-gradient-to-r from-amber-500 to-orange-600'
        }`}>
          <div className="flex items-center gap-6">
            <div className="bg-white p-3 rounded-full animate-bounce">
               {notification?.type === 'emergency' ? <AlertTriangle className="w-10 h-10 text-red-600" /> : 
                notification?.type === 'transfer' ? <ArrowRightLeft className="w-10 h-10 text-indigo-600" /> :
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className={`${isZoomed ? 'w-2/3' : 'w-1/3'} bg-slate-100/5 backdrop-blur-sm border-l border-slate-700 p-4 flex flex-col gap-4 overflow-y-auto custom-scrollbar transition-all duration-300`}>
          {screenClinics.map((clinic) => (
            <div key={clinic.id} className={`relative overflow-hidden rounded-xl border transition-all duration-300 shadow-lg ${clinic.is_active ? 'bg-white border-blue-100 transform hover:scale-[1.01]' : 'bg-slate-200 border-slate-300 opacity-60 grayscale'}`}>
