        ...
        <div className={`shadow-2xl border-b-8 border-white p-6 flex justify-center items-center h-32 ${notification?.type === 'emergency' ? 'bg-red-700' : notification?.type === 'transfer' ? 'bg-indigo-700' : 'bg-gradient-to-r from-amber-500 to-orange-600'}`}>
          <div className="flex items-center gap-6 w-full max-w-7xl justify-center">
            <div className="bg-white p-3 rounded-full animate-bounce shrink-0">
               {notification?.type === 'emergency' ? <AlertTriangle className="w-12 h-12 text-red-600" /> : notification?.type === 'transfer' ? <ArrowRightLeft className="w-12 h-12 text-indigo-600" /> : <Bell className="w-12 h-12 text-orange-600" />}
            </div>
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-black text-white drop-shadow-lg whitespace-nowrap overflow-hidden text-ellipsis leading-tight">{notification?.message}</h2>
          </div>
        </div>
      </div>
              ...
              <div key={clinic.id} className={`relative overflow-hidden rounded-xl border transition-all duration-300 shadow-lg ${notification?.targetClinicId === clinic.id ? 'animate-flash z-10 scale-105' : ''} ${clinic.is_active ? 'bg-white border-blue-100' : 'bg-slate-200 border-slate-300 opacity-60 grayscale'}`}>
                ...
                <div className="p-5 pr-8">
                  ...
                  <div className="flex justify-between items-center border-t border-slate-100 pt-3">
                    ...
                    <div className="flex items-center gap-1 text-slate-400 font-mono text-lg"><Clock className="w-5 h-5" /><span>{clinic.last_call_time ? new Date(clinic.last_call_time).toLocaleTimeString('ar-EG', {hour: '2-digit', minute:'2-digit'}) : '--:--'}</span></div>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="h-48 bg-gradient-to-r from-slate-900 to-slate-800 border-t-4 border-slate-700 relative overflow-hidden shrink-0">
             {doctors.length > 0 && currentDoctor ? (
               <div key={currentDoctor.id} className="h-full flex items-center p-6 animate-slide-in-right">
                 <div className="w-32 h-32 rounded-full border-4 border-blue-500 overflow-hidden shadow-2xl flex-shrink-0 bg-white flex items-center justify-center text-slate-400 relative z-10"><User className="w-16 h-16" /></div>
