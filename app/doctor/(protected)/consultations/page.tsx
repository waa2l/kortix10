...
type Consultation = {
  ...
  transfer_note?: string; // ملاحظة التحويل
  patient?: PatientData;
};
...
export default function DoctorConsultationsPage() {
  ...
  // --- 1. جلب بيانات الطبيب والقوائم ---
  useEffect(() => {
    const init = async () => {
      const { data: { user } } = await supabase.auth.getUser();
      if (user) {
        // نفترض جدول doctors مرتبط بـ auth.users
  ...
  // --- 2. جلب الاستشارات (فلترة حسب التخصص والتبويب) ---
  useEffect(() => {
    if (currentDoctor) {
      fetchConsultations();
    }
  ...
  const fetchConsultations = async () => {
    ...
    if (activeTab === 'inbox') {
      // واردة: مفتوحة + تتبع تخصص الطبيب
      query = query
        .eq('status', 'open')
        .eq('specialization', currentDoctor?.specialization); 
    } else {
      // أرشيف: مغلقة + رد عليها هذا الطبيب
      query = query
        .eq('status', 'completed')
  ...
  const fetchSettings = async () => {
    ...
    else setAvailableSpecializations(['باطنة', 'أطفال', 'عظام', 'جلدية']);
  };
  ...
  // التحويل (مع تسجيل اسم الطبيب والملاحظة)
  const handleTransfer = async (targetSpec: string) => {
    ...
    const { error } = await supabase.from('consultations')
      .update({ specialization: targetSpec, transfer_note: note })
      .eq('id', selectedConsultation.id);
    
    if (!error) { alert(`تم التحويل إلى ${targetSpec}`); handleSkip(); } 
    else { alert('حدث خطأ'); }
  };
  ...
  const handleReport = async () => {
    if(confirm('هل أنت متأكد من الإبلاغ عن هذه الاستشارة كإساءة؟')) {
       await supabase.from('consultations')
         .update({ status: 'flagged', is_flagged: true })
         .eq('id', selectedConsultation?.id);
       handleSkip();
    }
  };
  ...
  const startReply = () => {
    setStep(1);
    setReplyData({ diagnosis: '', selectedTests: [], selectedImaging: [], prescriptions: [], healthMessages: [], followUpDate: '', notes: '' });
    setViewMode('reply');
  };
  ...
  const removeMedicine = (index: number) => {
    const newMeds = [...replyData.prescriptions]; newMeds.splice(index, 1); setReplyData({ ...replyData, prescriptions: newMeds });
  };
  ...
  const handleSubmitReply = async () => {
    ...
    const { error } = await supabase.from('consultations').update({
       status: 'completed',
       doctor_id: currentDoctor.id, // تسجيل الطبيب القائم بالرد
    ...
    if(!error) { alert('تم الإرسال بنجاح'); handleSkip(); } 
    else { alert('حدث خطأ'); }
  };
      ...
      {viewMode === 'reply' && (
        <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl border border-slate-200">
          ...
          <div className="p-8 min-h-[400px]">
            {/* ... Steps 1 to 6 (نفس الكود السابق تماماً) ... */}
            {step === 1 && (<div className="space-y-4 animate-in fade-in"><h3 className="text-xl font-bold text-slate-800 mb-4">1. التشخيص المبدئي</h3><input list="d" className="w-full p-3 border rounded-xl" value={replyData.diagnosis} onChange={e=>setReplyData({...replyData, diagnosis:e.target.value})}/><datalist id="d">{diagnosesList.map((d,i)=><option key={i} value={d.diagnosis_name}/>)}</datalist></div>)}
            {step === 2 && (<div className="space-y-4 animate-in fade-in"><h3 className="text-xl font-bold text-slate-800 mb-4">2. التحاليل المطلوبة</h3><div className="flex gap-2 mb-4"><select className="flex-1 p-3 border rounded-xl" id="ts"><option value="">-- اختر تحليل --</option>{testsList.map((t,i)=><option key={i} value={t.test_name}>{t.test_name}</option>)}</select><button onClick={()=>{const val=(document.getElementById('ts') as HTMLSelectElement).value; if(val && !replyData.selectedTests.includes(val)) setReplyData({...replyData, selectedTests:[...replyData.selectedTests, val]})}} className="bg-blue-600 text-white p-3 rounded-xl"><Plus/></button></div><div className="flex flex-wrap gap-2">{replyData.selectedTests.map((t,i)=><span key={i} className="bg-blue-50 text-blue-700 px-3 py-1 rounded-full text-sm flex items-center gap-2">{t} <button onClick={()=>setReplyData({...replyData, selectedTests:replyData.selectedTests.filter((_,x)=>x!==i)})}><X className="w-3 h-3"/></button></span>)}</div></div>)}
            {step === 3 && (<div className="space-y-4 animate-in fade-in"><h3 className="text-xl font-bold text-slate-800 mb-4">3. الأشعات المطلوبة</h3><div className="flex gap-2 mb-4"><select className="flex-1 p-3 border rounded-xl" id="img"><option value="">-- اختر أشعة --</option>{imagingList.map((t,i)=><option key={i} value={t.imaging_name}>{t.imaging_name}</option>)}</select><button onClick={()=>{const val=(document.getElementById('img') as HTMLSelectElement).value; if(val && !replyData.selectedImaging.includes(val)) setReplyData({...replyData, selectedImaging:[...replyData.selectedImaging, val]})}} className="bg-blue-600 text-white p-3 rounded-xl"><Plus/></button></div><div className="flex flex-wrap gap-2">{replyData.selectedImaging.map((t,i)=><span key={i} className="bg-purple-50 text-purple-700 px-3 py-1 rounded-full text-sm flex items-center gap-2">{t} <button onClick={()=>setReplyData({...replyData, selectedImaging:replyData.selectedImaging.filter((_,x)=>x!==i)})}><X className="w-3 h-3"/></button></span>)}</div></div>)}
            {step === 4 && (<div className="space-y-4 animate-in fade-in"><h3 className="text-xl font-bold text-slate-800 mb-4">4. الوصفة الطبية</h3><div className="bg-slate-50 p-4 rounded-xl border border-slate-200 grid grid-cols-2 md:grid-cols-4 gap-3"><div className="col-span-2 md:col-span-4"><label className="text-xs font-bold text-slate-500">الدواء</label><input list="mds" className="w-full p-2 border rounded" value={tempMed.name} onChange={e=>setTempMed({...tempMed, name:e.target.value})}/><datalist id="mds">{medicinesList.map((m,i)=><option key={i} value={m.medicine_name}/>)}</datalist></div>{/* ... باقي حقول الدواء ... */}<div className="col-span-2 md:col-span-1 flex items-end"><button onClick={addMedicine} className="w-full bg-green-600 text-white py-2 rounded font-bold">إضافة</button></div></div><div className="space-y-2 mt-4">{replyData.prescriptions.map((m,i)=><div key={i} className="flex justify-between items-center bg-white p-3 border rounded-lg shadow-sm"><p className="font-bold text-slate-800">{m.name}</p><button onClick={()=>removeMedicine(i)} className="text-red-500"><Trash2 className="w-4 h-4"/></button></div>)}</div></div>)}
            {step === 5 && (<div className="space-y-4 animate-in fade-in"><h3 className="text-xl font-bold text-slate-800 mb-4">5. التثقيف الصحي</h3><div className="flex gap-2 mb-4"><select className="flex-1 p-3 border rounded-xl" id="hm"><option value="">-- اختر رسالة --</option>{messagesList.map((m,i)=><option key={i} value={m.message_text}>{m.message_text}</option>)}</select><button onClick={()=>{const val=(document.getElementById('hm') as HTMLSelectElement).value; if(val && !replyData.healthMessages.includes(val)) setReplyData({...replyData, healthMessages:[...replyData.healthMessages, val]})}} className="bg-green-600 text-white p-3 rounded-xl"><Plus/></button></div><div className="space-y-2">{replyData.healthMessages.map((m,i)=><div key={i} className="flex justify-between items-center bg-green-50 p-3 rounded-lg border border-green-100"><p className="text-sm text-green-800">{m}</p><button onClick={()=>setReplyData({...replyData, healthMessages:replyData.healthMessages.filter((_,x)=>x!==i)})}><Trash2 className="w-4 h-4 text-red-500"/></button></div>)}</div></div>)}
            {step === 6 && (<div className="space-y-4 animate-in fade-in"><h3 className="text-xl font-bold text-slate-800 mb-4">6. المتابعة والملاحظات</h3><div><label className="text-sm font-bold text-slate-600 mb-2 block">تاريخ المتابعة</label><input type="date" className="w-full p-3 border rounded-xl" value={replyData.followUpDate} onChange={e=>setReplyData({...replyData, followUpDate:e.target.value})}/></div><div className="mt-4"><label className="text-sm font-bold text-slate-600 mb-2 block">ملاحظات</label><textarea className="w-full p-4 border rounded-xl h-32" value={replyData.notes} onChange={e=>setReplyData({...replyData, notes:e.target.value})}/></div></div>)}
               ...
               <div className="animate-in fade-in">
                  <div className="flex justify-between items-center mb-4 px-4">
                     ...
                     <div className="flex gap-2">
                        ...
                        {activeTab === 'inbox' && (
                          <button onClick={handleSubmitReply} className="bg-green-600 text-white px-6 py-2 rounded-lg font-bold flex items-center gap-2 shadow-lg shadow-green-200 hover:bg-green-700"><CheckCircle className="w-4 h-4"/> حفظ وإرسال</button>
                        )}
                     </div>
                  </div>
