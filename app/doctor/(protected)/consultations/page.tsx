...
import { 
  ...
  Send, X, Share2, MessageSquare, Plus, Trash2, CheckCircle, FileText
} from 'lucide-react';
import { toArabicNumbers } from '@/lib/utils'; // تأكد أن هذه الدالة موجودة أو استخدم دالة بسيطة
...
type Consultation = {
  ...
  patient?: { full_name: string; gender: string; age: number }; // افترضنا وجود ربط
};
...
export default function DoctorConsultationsPage() {
  ...
  const [replyData, setReplyData] = useState({
    ...
    prescriptions: [] as any[], // Complex Medicine Objects
    healthMessage: '',
    followUpDate: '',
    notes: ''
  });
  ...
  const fetchLists = async () => {
    // جلب القوائم المنسدلة دفعة واحدة
  ...
  const handleSkip = () => {
    // تخطي محلياً فقط (إخفاء من القائمة الحالية)
  ...
  const handleTransfer = async () => {
    ...
    if (newSpec && selectedConsultation) {
      ...
      handleSkip(); // إخفاء بعد التحويل
    }
  };

  const handleReport = async () => {
    if(confirm('هل أنت متأكد من الإبلاغ عن هذه الاستشارة كإساءة؟')) {
       // يمكن إضافة حقل is_flagged في قاعدة البيانات
  ...
  const startReply = () => {
    setStep(1);
    setReplyData({
      diagnosis: '', selectedTests: [], selectedImaging: [], prescriptions: [], healthMessage: '', followUpDate: '', notes: ''
    });
    setViewMode('reply');
  };
  ...
  const handleSubmitReply = async () => {
    ...
    const { error } = await supabase.from('consultations').update({
       ...
       medicines: JSON.stringify(replyData.prescriptions), // حفظ الأدوية كـ JSON
       tests: replyData.selectedTests.join(', '),
       imaging: replyData.selectedImaging.join(', '),
       health_messages: replyData.healthMessage,
       // يمكن إضافة حقل follow_up_date في الجدول إذا أردت
      ...
      {viewMode === 'reply' && (
        <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl border border-slate-200">
          ...
          <div className="p-8 min-h-[400px]">
            ...
            {/* STEP 5: Health Message */}
            {step === 5 && (
               <div className="space-y-4 animate-in fade-in">
                  <h3 className="text-xl font-bold text-slate-800 mb-4">5. التثقيف الصحي</h3>
                  <div className="space-y-2">
                     <label className="text-sm font-bold text-slate-600">اختر رسالة جاهزة:</label>
                     <select 
                       className="w-full p-3 border rounded-xl"
                       onChange={e => setReplyData({...replyData, healthMessage: replyData.healthMessage + (replyData.healthMessage ? '\n' : '') + e.target.value})}
                     >
                        <option value="">-- اختر رسالة لإضافتها --</option>
                        {messagesList.map((m, i) => <option key={i} value={m.message_text}>{m.message_text}</option>)}
                     </select>
                     
                     <label className="text-sm font-bold text-slate-600 mt-4 block">نص الرسالة (يمكنك التعديل):</label>
                     <textarea 
                        className="w-full p-4 border rounded-xl h-32" 
                        value={replyData.healthMessage}
                        onChange={e => setReplyData({...replyData, healthMessage: e.target.value})}
                     ></textarea>
                  </div>
               </div>
            )}
               ...
               <div className="space-y-4 animate-in fade-in">
                  ...
                  <div>
                     ...
                     <input 
                        ...
                        onChange={e => setReplyData({...replyData, followUpDate: e.target.value})}
                     />
                  </div>
                  <div className="mt-4">
                     ...
                     <textarea 
                        ...
                        onChange={e => setReplyData({...replyData, notes: e.target.value})}
            ...
            {step === 7 && (
               <div className="space-y-4 animate-in fade-in text-center">
                  ...
                  <h3 className="text-2xl font-bold text-slate-800">مراجعة وإرسال</h3>
                  <p className="text-slate-500">هل أنت متأكد من البيانات المدخلة وإرسال الرد للمريض؟</p>
                  
                  <div className="bg-slate-50 text-right p-4 rounded-xl text-sm space-y-2 mt-4 mx-auto max-w-lg">
                     <p><strong>التشخيص:</strong> {replyData.diagnosis}</p>
                     <p><strong>الأدوية:</strong> {replyData.prescriptions.length} صنف</p>
                     <p><strong>التحاليل:</strong> {replyData.selectedTests.length}</p>
                  </div>
               </div>
            )}
          ...
          <div className="p-4 border-t border-slate-200 flex justify-between">
             ...
             {step < 7 ? (
                ...
                <button onClick={handleSubmitReply} className="px-8 py-2 rounded-lg bg-green-600 text-white font-bold hover:bg-green-700 flex items-center gap-2">
